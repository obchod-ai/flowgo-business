...
export function calculatePrice(distanceKm, region) {
  if (region === "Praha") {
...
export async function createFlowGoQuote(pickupAddress, deliveryAddress) {
  const apiKey = process.env.GOOGLE_ROUTES_API_KEY;

  if (!apiKey) {
    throw new Error("Na serveru chybí GOOGLE_ROUTES_API_KEY.");
  }
  ...
  if (!allowedRegions.includes(delivery.region)) {
    return {
      ...
      message: "FlowGo je dostupné pouze pro Prahu a Středočeský kraj.",
    };
  }
  ...
  return {
    ...
    price: calculatePrice(distanceKm, pricingRegion),
    currency: "CZK",
  };
}